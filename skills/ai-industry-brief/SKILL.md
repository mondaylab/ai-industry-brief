...
1. 开场先回答“今天具体发生了什么”，控制在 50 个中文字符以内。必须点出至少 2 个当天主体、产品或事件，并使用 `发布`、`接入`、`融资`、`开源`、`更新`、`上线` 等事实动词；尽量覆盖 2 个不同栏目。
   - 开场是事件摘要，不是每天重复的产业阶段判断。默认禁止以 `AI 正从`、`AI 开始`、`AI 进入`、`AI 转向`、`AI 走向`、`AI 的竞争` 开头，也避免反复使用“从 A 到 B”“正在重写”“进入新阶段”等万能句式。
   - 写完后对比最近 7 期开场；如果句法骨架或抽象名词高度相似，改为当天公司、产品、融资或研究动作。示例：`Google 更新 Pixel 和健康监测，ChatGPT 新增订餐，Agent 研究关注长期维护。`
2. 12 条动态都写成可独立读懂的完整短新闻句，使用“公司/产品 + 明确谓语动词 + 对象”结构，不使用 `|` 分隔主语和动作。
3. 每条描述先写事实，再写行业影响，长度约 60-80 个中文字符。
4. 写一段跨栏目洞察，控制在 150 个中文字符以内。先补充开场未展开的具体事件，再说明这些事件共同意味着什么；避免模糊套话和只描述“AI 又进入某阶段”。
