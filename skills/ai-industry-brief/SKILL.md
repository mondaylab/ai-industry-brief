...
1. Generate `briefs/YYYY-MM-DD.html` using the weekday palette and fixed branding from the spec.
...
2. Open the archive and new detail page in a browser; confirm card navigation, header, footer, wrapping, and small-screen readability.
3. When publishing is requested or already configured, commit only relevant site and skill files, push to the configured GitHub repository, and verify the GitHub Pages URL after deployment.
4. When a scheduled automation drives the workflow, keep its branding, footer, archive, and publish instructions synchronized with this skill.
